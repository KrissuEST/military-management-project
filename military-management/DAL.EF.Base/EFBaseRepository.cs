...
// Just wrapper around entity framework methods. Encapsulated, only visible inside repository.
// Lots of generics, lots of interfaces, lots of overloads - get used to it.
...
public class EFBaseRepository<TEntity, TKey, TDbContext> : IBaseRepository<TEntity, TKey> 
    ...
    where TDbContext : DbContext  //constraint here for Set
{
    protected TDbContext RepositoryDbContext;   //protected - accessable from classes and subclasses
    ...
    public virtual async Task<IEnumerable<TEntity>> AllAsync()  // virtual - everything can be replaced
    {
